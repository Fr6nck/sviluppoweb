<?php use function MHW\b; use MHW\{Support, Csrf}; $title = 'Clienti'; $nav = 'clienti'; ?>
<h1>Clienti</h1>
<table style="margin-top:24px">
  ...
  <?php foreach ($rows as $row): ?>
    <tr>
      <td><a href="<?= b() ?>/admin/cliente/<?= (int) $row['account_id'] ?>"><strong><?= Support::e($row['name'] ?: '—') ?></strong></a>
      ...
      <td style="text-align:right">
        <form method="post" action="<?= b() ?>/admin/entra/<?= (int) $row['user_id'] ?>" style="margin:0"><?= Csrf::field() ?>
