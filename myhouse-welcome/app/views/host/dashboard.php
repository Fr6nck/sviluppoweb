<?php use MHW\{Support, Csrf}; $title = $p['name']; $nav = 'pannello'; ?>
<div class="spread">
  <div>
    ...
    <p class="row small" style="margin-top:10px">
      ...
      <span class="muted"><a href="/g/<?= Support::e($p['slug']) ?>">/g/<?= Support::e($p['slug']) ?></a></span>
    </p>
  </div>
  <form method="post" action="/pannello/<?= (int) $p['id'] ?>/pubblica"><?= Csrf::field() ?>
...
<nav class="nav" style="margin-top:24px;display:inline-flex">
  <a class="on" href="/pannello/<?= (int) $p['id'] ?>">Sezioni</a>
...
<div class="spread" style="margin-top:24px">
  <h2>Le sezioni</h2>
  <form method="post" action="/pannello/<?= (int) $p['id'] ?>/sezioni/nuova"><?= Csrf::field() ?>
    <button class="btn btn--ghost btn--sm">Aggiungi sezione</button></form>
</div>
<div class="list" style="margin-top:12px">
  <?php foreach ($sections as $s): ?>
    <a class="list-item" href="/pannello/<?= (int) $p['id'] ?>/sezioni/<?= (int) $s['id'] ?>">
